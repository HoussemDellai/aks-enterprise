...
#   provisioner "local-exec" {
#     command = <<-EOT
#       az aks update -g ${azurerm_kubernetes_cluster.aks.resource_group_name} -n ${azurerm_kubernetes_cluster.aks.name} --enable-image-cleaner
#     EOT
#   }
...
#     interpreter = ["PowerShell", "-Command"]
#     command     = <<-EOT
#       az aks update -g ${azurerm_kubernetes_cluster.aks.resource_group_name} `
#                     -n ${azurerm_kubernetes_cluster.aks.name} `
#                     --enable-apiserver-vnet-integration `
#                     --apiserver-subnet-id ${azurerm_subnet.subnet_apiserver.id}
#     EOT
#   }
