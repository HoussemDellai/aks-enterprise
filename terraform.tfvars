# prefix = "demo041"
rg_hub                            = "rg-hub"
rg_spoke                          = "rg-spoke"
rg_spoke_mgt                      = "rg-spoke-mgt"
rg_aks                            = "rg-aks-cluster" #TODO
rg_aks_nodes                      = "rg-aks-cluster-managed"
resources_location                = "westeurope"
aks_name                          = "aks-cluster"
acr_name                          = "acrforakstf0111"
keyvault_name                     = "kvforaks0115"
log_analytics_workspace           = "loganalyticsakscluster"
kubernetes_version                = "1.24.6"
aks_network_plugin                = "azure" # "kubenet"
aad_group_aks_admins              = "group_aks_admins"
cidr_vnet_hub                     = ["172.17.0.0/16"]
cidr_subnet_firewall              = ["172.17.0.0/26"]
cidr_vnet_spoke                   = ["10.0.0.0/16"]
cidr_subnet_appgateway            = ["10.1.0.0/16"]
cidr_subnet_pe                    = ["10.2.0.0/28"]
cidr_subnet_bastion               = ["10.3.0.0/27"]
cidr_subnet_pods                  = ["10.240.0.0/16"]
cidr_subnet_nodes                 = ["10.241.0.0/24"]
cidr_vnet_spoke_mgt = ["10.1.0.0/16"]
cidr_subnet_mgt = ["10.1.0.0/24"]
cidr_aks_service                  = "10.0.0.0/16"
cidr_aks_docker_bridge            = "172.17.0.1/16"
aks_dns_service_ip                = "10.0.0.10"
aks_outbound_type                 = "userAssignedNATGateway" # "loadBalancer" # "loadBalancer" # userDefinedRouting, managedNATGateway
enable_app_gateway                = false                    # 
enable_private_cluster            = true                     # true  #
enable_vnet_peering               = true
enable_monitoring                 = true
enable_apiserver_vnet_integration = false # true           # 
enable_nodepool_apps              = true
enable_nodepool_spot              = false # false
enable_aks_admin_group            = true
enable_aks_admin_rbac             = true
enable_private_acr                = true
enable_keyvault                   = false
enable_private_keyvault           = false #TODO
enable_bastion                    = true
enable_firewall                   = true
enable_vm_jumpbox_windows         = true
enable_vm_jumpbox_linux           = true
# enable_velero_backups             = true
# aks_admin_group_object_ids        = ["1eb16a7c-42cc-49e3-8ff0-d179433be6a6"] # HoussemDellaiGroup
# storage_account_name_backup       = "storageforaksbackup001"
# backups_rg_name                   = "rg-aks-cluster-backups"
# backups_region                    = "northeurope"